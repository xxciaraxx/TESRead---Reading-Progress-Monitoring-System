...
.nav-section-label { font-size: 9.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.4px; color: rgba(255,255,255,0.28); padding: 12px 10px 5px; font-family: 'Sora', sans-serif; }
.nav-item { display: flex; align-items: center; gap: 9px; padding: 9px 11px; border-radius: var(--radius-sm); color: rgba(255,255,255,0.65); font-weight: 500; font-size: 13px; transition: all 0.18s ease; margin-bottom: 1px; position: relative; cursor: pointer; }
...
.nav-item.active::before { content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 3px; height: 20px; background: linear-gradient(180deg, #fff, rgba(255,255,255,0.6)); border-radius: 0 3px 3px 0; }
.nav-item i { width: 17px; font-size: 14px; flex-shrink: 0; opacity: 0.85; text-align: center; }
.nav-item.active i { opacity: 1; }
.nav-badge { margin-left: auto; background: var(--red); color: #fff; font-size: 9.5px; font-weight: 700; padding: 2px 6px; border-radius: 99px; min-width: 18px; text-align: center; box-shadow: 0 2px 6px rgba(200,16,46,0.4); }

/* SIDEBAR FOOTER - single sign out */
.sidebar-footer { padding: 12px 10px 14px; flex-shrink: 0; position: relative; }
.sidebar-footer::before { content: ''; position: absolute; top: 0; left: 18px; right: 18px; height: 1px; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.10), transparent); }
.sidebar-user-card { display: flex; align-items: center; gap: 9px; padding: 9px 10px; border-radius: var(--radius-sm); margin-bottom: 8px; }
.sidebar-avatar { width: 34px; height: 34px; border-radius: 50%; object-fit: cover; border: 2px solid rgba(255,255,255,0.25); flex-shrink: 0; }
...
.btn-signout { display: flex; align-items: center; justify-content: center; gap: 8px; width: 100%; padding: 9px 14px; background: rgba(200,16,46,0.18); border: 1px solid rgba(200,16,46,0.30); border-radius: var(--radius-sm); color: rgba(255,180,180,0.9); font-size: 12.5px; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; transition: all 0.18s; text-align: center; }
...
/* HEADER */
.top-header { position: sticky; top: 0; height: var(--header-h); background: rgba(255,255,255,0.88); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border-bottom: 1px solid var(--border); display: flex; align-items: center; padding: 0 24px; z-index: 90; gap: 14px; box-shadow: 0 1px 0 var(--border), 0 4px 24px rgba(0,30,100,0.05); }
.header-page-title { font-size: 15px; font-weight: 700; color: var(--text); flex: 1; display: flex; align-items: center; gap: 9px; font-family: 'Sora', sans-serif; letter-spacing: -0.2px; }
.page-icon { width: 30px; height: 30px; background: var(--primary-pale); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 14px; border: 1px solid rgba(0,58,140,0.10); flex-shrink: 0; }
.header-date { color: var(--muted); font-size: 12px; font-weight: 500; display: flex; align-items: center; gap: 5px; background: var(--primary-pale); border: 1px solid rgba(0,58,140,0.10); border-radius: 6px; padding: 5px 11px; }
.header-date i { color: var(--primary); font-size: 11px; opacity: 0.7; }
.header-actions { display: flex; align-items: center; gap: 8px; }
.header-user-pill { display: flex; align-items: center; gap: 8px; background: var(--primary-pale); border: 1px solid rgba(0,58,140,0.12); border-radius: 50px; padding: 5px 13px 5px 5px; cursor: default; }
...
.stat-card { background: var(--card); border-radius: var(--radius); padding: 20px 18px 18px; box-shadow: var(--shadow); border: 1px solid var(--border); position: relative; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; }
...
/* WELCOME BANNER */
.welcome-banner { background: linear-gradient(135deg, #002870 0%, #003A8C 40%, #1a3a6b 70%, #C8102E 100%); border-radius: var(--radius); padding: 26px 30px; display: flex; align-items: center; justify-content: space-between; margin-bottom: 22px; overflow: hidden; position: relative; }
...
.welcome-badge { display: flex; align-items: center; gap: 8px; background: rgba(255,255,255,0.14); border: 1px solid rgba(255,255,255,0.22); border-radius: 50px; padding: 7px 16px; color: #fff; font-size: 12.5px; font-weight: 600; margin-top: 12px; width: fit-content; backdrop-filter: blur(4px); }

/* TABLES */
.table-wrap { overflow-x: auto; }
.data-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.data-table thead th { background: #f7f9ff; font-weight: 700; font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.9px; color: var(--muted); padding: 12px 16px; text-align: left; border-bottom: 1.5px solid var(--border); white-space: nowrap; font-family: 'Sora', sans-serif; }
...
<div class="app-shell">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <div class="nav-section-label" style="margin-top:8px;">My Class</div>
            ...
            <a href="{{ route('teacher.assessments.index') }}" class="nav-item {{ request()->routeIs('teacher.assessments.*') ? 'active' : '' }}">
                <i class="fas fa-clipboard-check"></i><span>Assessments</span>
            </a>
            <a href="{{ route('teacher.interventions.index') }}" class="nav-item {{ request()->routeIs('teacher.interventions.*') ? 'active' : '' }}">
                <i class="fas fa-hands-helping"></i><span>Interventions</span>
                @php $activeInterv = \App\Models\Intervention::where('teacher_id', auth()->id())->where('status', 'Active')->count(); @endphp
                @if($activeInterv > 0)<span class="nav-badge">{{ $activeInterv }}</span>@endif
            </a>
            <a href="{{ route('teacher.reports.index') }}" class="nav-item {{ request()->routeIs('teacher.reports.*') ? 'active' : '' }}">
                <i class="fas fa-chart-line"></i><span>Analytics & Reports</span>
            </a>

            <div class="nav-section-label" style="margin-top:8px;">Account</div>
            <a href="{{ route('teacher.profile.edit') }}" class="nav-item {{ request()->routeIs('teacher.profile.*') ? 'active' : '' }}">
            ...
            <div class="header-actions">
                <div class="header-user-pill">
                    ...
                    <div class="header-user-info">
                        ...
                        <span class="header-user-role">Teacher</span>
                    </div>
                </div>
            </div>
        </header>
